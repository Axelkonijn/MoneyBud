...
| **Mapping of building blocks** | Everything, into that single process. Nothing is distributed, so there is no allocation decision to make. [§5](05-building-block-view.md) is still empty, and when it is written it will not change this |
...
|---|---|
| How data is stored on the machine — file, embedded database, something else — and where it lives | [§8.3](08-crosscutting-concepts.md), which is empty |
