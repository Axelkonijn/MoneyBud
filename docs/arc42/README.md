...
| [08](08-crosscutting-concepts.md) | Cross-cutting Concepts | Rules applying everywhere — money handling lives here | **Filled** — §8.1 maps §12's two distinctions onto the code, places income's pool outside both of them, and lists what has no code yet (*Assign* and *Over-assigned* now lead that list; income and *Unassigned* have left it, built); §8.2 settles whole cents, the `Money` type, the sign convention and no currency field — unchanged by income, which those rules already covered — with storage and timezones still open; §8.3 defers persistence, with the trigger stated |
