...
| [08](08-crosscutting-concepts.md) | Cross-cutting Concepts | Rules applying everywhere — money handling lives here | **Filled** — §8.1 maps §12's two distinctions onto the code, places income's pool outside both of them, and lists what has no code yet (*Assign* and *Over-assigned* lead that list, joined now by *Archived* and the default categories; income and *Unassigned* have left it, built). §8.1 also carries **one live divergence between the code and §12**: category names are compared exactly in `Ledger` — case-sensitively, in two places, and without trimming — by accident rather than by decision, where §12 now decides trimmed and case-insensitive, with a blank name refused. §8.2 settles whole cents, the `Money` type, the sign convention and no currency field — unchanged by income, which those rules already covered — with storage and timezones still open; §8.3 defers persistence, with the trigger stated |
