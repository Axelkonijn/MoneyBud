...
| **`src/MoneyBud.Desktop`** | The Avalonia window, laid out income, plan, expenses, and `RingControl`, which draws the ring from `Ring`'s shares and turns the pointer's position into a share. Transaction rows are buttons that load their entry, and each category row has its acts in a line under its name. Starts a first-start ledger on the system clock, and tells the screen to look again once a minute. **Deliberately thin, and with no automated tests except one that reads the window's markup as text** for the order of the form fields ([§8.4](08-crosscutting-concepts.md), [§11](11-risks-and-technical-debt.md)) |
